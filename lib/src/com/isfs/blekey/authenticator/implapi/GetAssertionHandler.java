...
public class GetAssertionHandler {
    ...
    public static byte[] getNextAssertion(CtapTxn txn, Map<Integer, Object> req) {
        ...
        if (!UxInteractionLock.get().isUserDenied()) {
            logger.warn("getNextAssertion: Ux session already denied — OPERATION_DENIED");
    ...
    static byte[] executeGetAssertion(CtapTxn txn, Map<Integer, Object> req) throws Exception {
        ...
        if (platformKey == null) return CtapResponse.error(Ctap2StatusCode.OTHER);
        ...
        if (seed == null) return CtapResponse.error(Ctap2StatusCode.OTHER);
        ...
        if (credentials.isEmpty()) return CtapResponse.error(Ctap2StatusCode.NO_CREDENTIALS);
        ...
        if (selectedCredential == null) {
            ...
            return CtapResponse.error(Ctap2StatusCode.NO_CREDENTIALS);
        }
